...
export const clientCreateSchema = baseClientSchema.superRefine((data, ctx) => {
  if (data.type === 'PARTICULIER') {
    if (!data.first_name) {
      ctx.addIssue({
        ...
        message: 'Prénom requis pour un particulier',
        path: ['first_name'],
      });
    }
    if (!data.last_name) {
  ...
  if (data.type === 'PROFESSIONNEL') {
    if (!data.company_name) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Raison sociale requise pour un professionnel',
        path: ['company_name'],
      });
    }
    if (!data.contact_name) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Contact requis pour un professionnel',
        path: ['contact_name'],
      });
    }
  }
