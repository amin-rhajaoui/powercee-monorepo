...
def fill_tva_attestation(prospect_details: dict) -> bytes | None:
    ...
    Args:
        prospect_details: Dictionnaire contenant :
            - nom, prenom: Nom et prénom du client
            - numero, adresse, code_postal, ville: Adresse complète
            - telephone, email: Coordonnées
            - type_bien: "maison" ou "appartement"
            - statut_occupation: "proprietaire" ou "locataire"
    
    Returns:
        Bytes du PDF rempli ou None en cas d'erreur
    """
    try:
        template_path = str(TEMPLATE_TVA_PATH)
        
        if not os.path.exists(template_path):
            logger.error(f"Template TVA introuvable: {template_path}")
            return None
...
def fill_cdc_cee_pdf(prospect_details: dict, pump_details: dict, devis_id: int) -> bytes | None:
    ...
    Args:
        prospect_details: Dictionnaire contenant :
            ...
            - numero, adresse, code_postal, ville: Adresse complète
            - telephone, email: Coordonnées
        pump_details: Dictionnaire contenant :
            - prime_cee: Montant de la prime CEE
        devis_id: Numéro de devis (int ou str)
    ...
    try:
        ...
        if not os.path.exists(template_path):
            logger.error(f"Template CDC introuvable: {template_path}")
            return None
        ...
        prime_cee = float(pump_details.get('prime_cee', 0.0))
        can.drawString(250, 640, f"{prime_cee:.2f} €")
        ...
        with open(template_path, "rb") as f:
            ...
            writer = PdfWriter()
            page = existing_pdf.pages[0]
            # Fusionne la surcouche par-dessus la page de base
            page.merge_page(overlay_pdf.pages[0])
            writer.add_page(page)
